  ...
  describe("ActsErrorMessageTest", () => {
    test("message", (t) => {
      t.throws(() => receiveString(function a() {} as ESObject), {
        message: "Failed to convert JavaScript value `function a(..) ` into rust type `String`",
      });
      ...
      t.throws(() => receiveString(1 as ESObject), {
        message: "Failed to convert JavaScript value `Number 1 ` into rust type `String`",
      });
      t.throws(
        () =>
          receiveString({
      ...
      t.throws(() => receiveString(100n as ESObject), {
        message: "Failed to convert JavaScript value `BigInt 100 ` into rust type `String`",
      });
    });
  });
}
