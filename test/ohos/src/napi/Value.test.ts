...
import { buffer } from "../utils/shims/arkts.test";
...
export default (path) => {
  describe("ValueTest", () => {
    ...
    test("function call", async (t) => {
      ...
      t.is(ctx3.name, "可乐");
      const cbSpy = spy();
      await createReferenceOnFunction(cbSpy.claser.test as ESObject);
    ...
    test("promise", async (t) => {
      ...
      t.is(cat, "cat");
      const cbs = spy();
      await callFinallyOnPromise(Promise.resolve(1), cbs.func as ESObject);
      cbs.mocker.verify("test", []).times(1);
    });
    ...
    test("reset empty buffer", (t) => {
      ...
      t.deepEqual(empty.byteLength, 0);
    });

    test("empty typed array", (t) => {
      t.is(derefUint8Array(getEmptyTypedArray(), new Uint8ClampedArray([])), 0);
    });
    ...
    test("buffer passthrough", async (t) => {
      const fixture = buffer.from("hello world").buffer;
    ...
    test("create BigInt", (t) => {
      t.is(createBigInt(), BigInt("-3689348814741910323300"));
    });

    test("create BigInt i64", (t) => {
      t.is(createBigIntI64(), BigInt(100));
    });

    test("BigInt get_u64", (t) => {
      t.is(bigintGetU64AsString(BigInt(0)), "0");
    });

    test("js mod test", (t) => {
      t.is(xxh64Alias(buffer.from("hello world").buffer), BigInt("1116"));
    ...
    test("throw error from ThreadsafeFunction", async (t) => {
      const throwPromise = new Promise((_, reject) => {
        threadsafeFunctionThrowError(reject);
      });
      try {
        await t.throwsAsync(throwPromise);
      } catch (err) {
        t.is(err?.message, "ThrowFromNative");
      }
    });

    test("ThreadsafeFunction closure capture data", (t) => {
      return new Promise((resolve) => {
        threadsafeFunctionClosureCapture(() => {
          resolve();
        });
    ...
    test("resolve value from thread safe function fatal mode", async (t) => {
    ...
    test("accept ThreadsafeFunction Fatal", async (t) => {
    ...
    test("accept ThreadsafeFunction tuple args", async (t) => {
      await new Promise<void>((resolve, reject) => {
    ...
    test("Class with getter setter closures", (t) => {
    ...
    test("Date to chrono::NativeDateTime test", (t) => {
      const fixture = new Date();
      t.is(chronoNativeDateTime(fixture), fixture.valueOf());
    });

    test("Date from chrono::NativeDateTime test", (t) => {
