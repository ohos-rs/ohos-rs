import { test,describe } from '../utils/framework.test';
...
export default () => {
  describe("value test", () => {
    ...
    test('structured enum', (t) => {
      ...
      t.throws(() => validateStructuredEnum({ type2: 'unknown' } as ESObject))
      t.throws(() => validateStructuredEnum({ type2: 'Greeting' } as ESObject))
    })
    ...
    test('function call', async (t) => {
      ...
      t.is(ctx3.name, '可乐')
      const cbSpy = spy();
      await createReferenceOnFunction(cbSpy.func as ESObject)
      t.is(cbSpy.mocker.verify(), 1)
      t.is(
