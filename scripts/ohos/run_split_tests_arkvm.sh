...
TARGET_RELEASE_DIR="/work/target/arkvm-host/release"
CONTAINER_ARK_HOST_BUNDLE_DIR="${ARK_HOST_CONTAINER_MOUNT}"
CONTAINER_ARK_ACE_NAPI_LIB="${ARK_HOST_CONTAINER_MOUNT}/libace_napi.so"
CONTAINER_STUB_FILE="${ARK_HOST_CONTAINER_MOUNT}/stub.an"
...
run_docker() {
  docker run --rm \
    ...
    -e STUB_FILE="${CONTAINER_STUB_FILE}" \
    -e TARGET_RELEASE_DIR="${TARGET_RELEASE_DIR}" \
    -e TEST_TIMEOUT_SEC="${TEST_TIMEOUT_SEC}" \
    -e ARK_HOST_BUNDLE_DIR="${CONTAINER_ARK_HOST_BUNDLE_DIR}" \
    -e ARK_ACE_NAPI_LIB="${CONTAINER_ARK_ACE_NAPI_LIB}" \
...
build_host_libs() {
  ...
  run_docker /bin/bash /work/scripts/ohos/arkvm_build_host_libs.sh
}
...
run_compile() {
  run_docker /bin/bash /work/scripts/ohos/arkvm_compile_abc_in_docker.sh
}

run_suites() {
  echo "==> Ark host bundle: ${ARK_HOST_BUNDLE_DIR}"
  echo "==> Work root: ${WORK_ROOT}"
  run_docker /bin/bash /work/scripts/ohos/arkvm_run_suites_in_docker.sh
}
